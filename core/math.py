...
def _validate_graph(nodes, edges, retraction_event_node_id):
    node_ids = [node["id"] for node in nodes]
    known_nodes = set(node_ids)

    if len(node_ids) != len(known_nodes):
        raise ValueError("Graph contains duplicate node ids.")
    if retraction_event_node_id not in known_nodes:
        raise ValueError(f"Unknown retraction event node id: {retraction_event_node_id}")
...
def propagate_retraction_gravity(nodes, edges, retraction_event_node_id, gravity_attenuation=0.8):
    ...
    """
    _validate_graph(nodes, edges, retraction_event_node_id)
    ...
    queue = deque([retraction_event_node_id])
    visited = set()
